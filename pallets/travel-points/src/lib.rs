...
pub mod pallet {
	...
	/// Trait for smart contract integration.
	/// Smart contracts can use these functions to interact with the travel points system.
	impl<T: Config> Pallet<T> {
		/// Interface for smart contracts to award points.
		/// The smart contract must be an authorized issuer.
		...
		/// Interface for smart contracts to check a user's point balance.
		pub fn contract_check_balance(user: &T::AccountId) -> u128 {
			Self::get_available_points(user)
		}

		/// Interface for smart contracts to check if an account is an authorized issuer.
